...
import com.thirutricks.tllplayer.legacy.ui.TvUiUtils
import com.thirutricks.tllplayer.legacy.ui.ModernToggleSwitch
import com.thirutricks.tllplayer.legacy.ui.GlassCard
import com.thirutricks.tllplayer.legacy.ui.GlassyBackgroundView
import com.thirutricks.tllplayer.legacy.ui.SettingsFocusManager
import com.thirutricks.tllplayer.legacy.ui.ResponsiveLayoutManager
import com.thirutricks.tllplayer.legacy.ui.SettingsAccessibilityManager
import com.thirutricks.tllplayer.legacy.ui.PerformanceOptimizationManager
import com.thirutricks.tllplayer.legacy.ui.SettingsResourceManager
...
import com.thirutricks.tllplayer.legacy.ui.SafeFragmentManager
...
import com.thirutricks.tllplayer.legacy.ui.glass.GlassType
...
class SettingFragment : Fragment() {
    ...
    private fun safeExitSettings() {
        try {
            if (::safeFragmentManager.isInitialized) {
                ...
                if (success) {
                    (activity as? LegacyMainActivity)?.showTime()
                } else {
    ...
    private fun setupListeners() {
        ...
        binding.switchChannelReversal.setOnCheckedChangeListener { _, b ->
            ...
            (activity as LegacyMainActivity).settingActive()
        }
        ...
        binding.switchProfile.setOnClickListener {
            ...
            CoroutineScope(Dispatchers.Main).launch {
                val settings = com.thirutricks.tllplayer.features.settings.data.SettingsRepository(ctx)
                settings.setActiveProfile(-1L)
                val intent = android.content.Intent(activity, com.thirutricks.tllplayer.MainActivity::class.java)
...
//        }
    ...
    private fun collectConfigs(): List<com.thirutricks.tllplayer.legacy.models.NetworkConfig> {
        val result = mutableListOf<com.thirutricks.tllplayer.legacy.models.NetworkConfig>()
        for (i in 0 until binding.configContainer.childCount) {
            val row = binding.configContainer.getChildAt(i)
            val input = row.findViewById<android.widget.EditText>(R.id.item_config_input)
            val text = input.text.toString().trim()
            if (text.isNotEmpty()) {
                val parsed = com.thirutricks.tllplayer.legacy.infrastructure.ConfigParser.identifyAndParse(text)
                result.add(parsed)
            }
        }
        return result
